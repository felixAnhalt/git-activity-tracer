...
- Naming: `camelCase` for vars/funcs/files, `PascalCase` for types/classes, `UPPER_SNAKE` for constants
- Prefer arrow functions; use `const` over `let` where possible
- Async/await: always use `async`/`await` over `.then()`;
- Rarely use classes when absolutely necessary for encapsulating state/behavior or implementing interfaces; usually use pure functions and modules
- Single responsibility: functions/classes should do one thing; split large functions (>50 lines) into smaller ones
